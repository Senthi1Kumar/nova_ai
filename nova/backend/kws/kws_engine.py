...
class GoogleEmbeddingModel:
    ...
    def predict(self, audio_16k: np.ndarray) -> np.ndarray:
        ...
        if len(audio_16k) < MIN_SAMPLES:
            audio_16k = np.pad(audio_16k, (0, MIN_SAMPLES - len(audio_16k)), mode='constant')
        if audio_16k.ndim == 1:
...
class StreamingKWS:
    """V4 Engine: DTW + MLP Classifier."""
    def __init__(self, model_path: str = None, threshold=0.25, window_sec=1.5, stride_sec=0.2):
        if model_path:
    ...
    async def process_chunk(self, audio_16k: np.ndarray):
        try:
            ...
            if not self.buffer.full: return False, 0
            self.new_samples += len(audio_16k)
            if self.new_samples < self.stride_size: return False, 0
            ...
            with torch.no_grad():
                for offset in [0, 5120, 10240, 15488]:
                    ...
                    if prob > 0.5:
                        ...
                        break
            if not speech_detected: return False, 0

            t_start = time.perf_counter()
            current_seq = self.model.predict(window)
            
            distances = [self._dtw_distance(current_seq, ref) for ref in self.reference_sequences]
            min_dist = min(distances) if distances else 1.0
            
            prob_wake = 0.0
            if self.mlp:
                feat = torch.FloatTensor(np.mean(current_seq, axis=0)).unsqueeze(0).to(next(self.mlp.parameters()).device)
                with torch.no_grad():
            ...
            if min_dist < self.threshold and prob_wake > 0.6:
                ...
                print(f"🔥 [KWS] VERIFIED (Dist: {min_dist:.2f}, Prob: {prob_wake:.2f})")
                return True, latency
        except Exception as e: print(f"[ERROR] KWS Processing failed: {e}")
        return False, 0
...
class EnrollmentManager:
    ...
    def record_sample(self, filename: str, prompt: str, duration=2.0):
        ...
        stream = self.pa.open(format=pyaudio.paInt16, channels=1, rate=self.sample_rate,
    ...
    def run_full_enrollment(self):
        prompts = ["Say 'Hi Nova'", "Say 'Nova'", "Say 'Hey Nova'", "Say 'Hello Nova'", "Say 'Nova'"]
        ref_paths = []
